...
bdev_nvme_reconnect_ctrlr_poll(void *arg)
{
	...
	if (!bdev_nvme_check_ctrlr_loss_timeout(nvme_ctrlr)) {
		rc = spdk_nvme_ctrlr_reconnect_poll_async(nvme_ctrlr->ctrlr);				// 原
		// nvme_ctrlr->size = sizeof(struct nvme_ctrlr);									// ljx添加
		// rc = spdk_nvme_ctrlr_reconnect_poll_async((struct spdk_nvme_ctrlr *)nvme_ctrlr);// ljx添加
		if (rc == -EAGAIN) {
			return SPDK_POLLER_BUSY;
		}
	}
